...
1. **信頼度フィルタ**: 既定しきい値 80 以上の指摘のみ採用（しきい値はユーザー指定可）
2. **コンセンサス検出**: 同じファイル・同じ行番号・類似観点で複数エージェントが指摘している項目は1件に統合し、`sources` フィールドに全エージェント名を列挙、信頼度は `max + min(20, (件数-1) * 5)` で補正（コンセンサスベース、100上限）
3. **重要度ソート**: Critical → Major → Minor の順、同重要度内では信頼度の降順
