...
### E3.5. 知見マーカーの追記（クラウド実行時のみ。直前の ship-local-only 分岐で E7 に進む前に必ず実行する）
...
2. 満たす場合、`gh pr view --repo <owner>/<repo> <PR番号> --json body -q .body` で現在の本文を取る。**`<!-- session-insight v1 -->` が既に含まれていれば、この手順は何もせず終える**（再 ship・途中失敗後の再実行で同じブロックが重複するのを防ぐ、fail-closed と同じ「迷ったら書かない」側）。含まれていなければ、E4 で書くはずだった内容（作業サマリー＝状態、技術知見＝知見）を Markdown で用意し、末尾に足して `gh pr edit --repo <owner>/<repo> <PR番号> --body-file` で書き戻す:
   ```markdown
   <!-- session-insight v1 -->
   ## 状態
...
4. 書いた・既存ブロックがあり何もしなかった・条件不成立で書かなかった、のいずれかを1行報告する
