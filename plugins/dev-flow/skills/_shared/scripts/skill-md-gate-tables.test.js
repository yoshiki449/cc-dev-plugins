...
// --- D5.5 収束チェック（thrashing 検知・Issue #78 相当）----------------------------
//
// D5（D4/D4.5/D4.6/D5 の反復）の収束条件は「Critical 0 件」だけで、周回の上限も
// 同一指摘の繰り返し検知も無かった。dev-loop の L1.3 は findings_hash の一致比較で
// 同じ問題を機械的に解決しているが、dev-verify 単体運用（/dev-loop を経由しない
// verify⇄fix の往復）にはその歯止めが無いため、レビュー基準がブレるケースで
// 「直す→また指摘→直す→また指摘」が収束しないまま続くことがあった（利用者実測の相談）。
// D5.5 として運用ルールを明文化した。dev-loop のような自動ハッシュ判定ではなく
// 人が2周分の指摘を突き合わせる運用ガードなので、字面固定の対象も「何を比較するか」
// 「何周で止めるか」「止めた後どうするか」の3点に絞る。
const D5_5_REQUIRED_LINES = [
  {
    key: '比較単位',
    line: '**同じ箇所で同じ種類の指摘が繰り返されているか**だけを見る',
    why: '行番号や言い回しの違いをそのまま「別の指摘」と数えると、実質的に同じ指摘が' +
      '文言の揺れだけで毎回「新規」扱いになり、収束チェックが機能しない',
  },
  {
    key: '停止条件',
    line: '**2周連続で同じ組が出たら、3周目の D5 に入らず止める**',
    why: '周回上限が無いと「Critical 0 件」を待つループが原理的に終わらない場合がある' +
      '（レビュアーの基準がブレている等、実装を直しても解消しない原因のとき）',
  },
  {
    key: 'Critical限定の前提',
    line: '**前提として、D5 ループの収束条件は「Critical 0 件」だけ**（Major は無制限にブロックしない）',
    why: 'この前提を読み手が知らないと、何周しても収束しない原因を「Major が残っているから」と' +
      '誤読し、収束チェックではなく Major 潰しを続けてしまう',
  },
  {
    key: 'dev-loopへの切替導線',
    line: '`/dev-loop` は本節と同じ収束チェックを findings のハッシュ比較として自動化しており（`halted-thrashing`）',
    why: '中〜大規模な往復が要る場面で単体 `/dev-verify` を回し続けると、本節の人手チェックより' +
      'dev-loop の機械判定のほうが向いていることに気づけない',
  },
  {
    key: 'エスカレーション手段の踏襲',
    line: '止めたら次のいずれかを選ぶ（`dev-fix` F2 のスコープ判断と同じ扱い）',
    why: '止めた後の受け皿が無いと、収束チェックが「止まるだけ」になり実務上使われなくなる。' +
      '既存の F2 スコープ判断（別Issue化/仕様確認/対応必須）を再利用する設計だと明示する',
  },
];

// 「手順4本体への接続」（D5 の手順4から D5.5 への参照）は D5.5 節そのものの外
// （親の D5 節）にある行なので、section('#### D5.5.') の範囲には入らない。
// 別テスト（直後の「D5 手順4が D5.5 を参照している」）で検査する。

test('AC: D5.5 の収束チェック（thrashing 検知）の挙動を決めている行が字面どおりに存在する', () => {
  assertRequiredLines(DEV_VERIFY_MD, '#### D5.5.', D5_5_REQUIRED_LINES, 'D5_5_REQUIRED_LINES', 5);
});

test('D5 手順4が D5.5 を参照している（収束チェックへの接続漏れ検出）', () => {
  const md = fs.readFileSync(DEV_VERIFY_MD, 'utf8');
  const d5 = section(md, '### D5.');
  assert.ok(d5.length > 100, 'D5 節の切り出しに失敗した');
  assert.ok(
    d5.includes('D5.5 の収束チェックを行う'),
    'D5 の手順4から D5.5 への参照が消えている。参照が無いと、ループの外に新設した' +
      '収束チェックが実際には一度も読まれない節になる',
  );
});
