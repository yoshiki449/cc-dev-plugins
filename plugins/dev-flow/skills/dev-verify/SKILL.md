...
4. 全テスト PASS かつ Critical 0 件になるまで D4 / D4.5 / D4.6 / D5 のループを繰り返す（2周目以降は D5.5 の収束チェックを行う）

#### D5.5. ループの収束チェック（thrashing 検知）⚠ 必須（スキップ不可・2周目以降のみ）

D5 のループ（D4 / D4.5 / D4.6 / D5 の反復）を2周以上回すときは、次のラウンドに入る前に**前回ラウンドの指摘と今回ラウンドの指摘を突き合わせる**。

1. 今回ラウンドの指摘を `ファイルパス × 指摘の主旨` の組で書き出す（E2E の FAIL メッセージ・`security-check-<Issue番号>.md` の Critical/Major 行が対象）
2. 前回ラウンドの組と比較する。行番号の違いや言い回しの違いは無視してよい。**同じ箇所で同じ種類の指摘が繰り返されているか**だけを見る
3. **2周連続で同じ組が出たら、3周目の D5 に入らず止める**。「実装のバグ」ではなく次のどれかを疑う:
   - レビュアー（human/security-tester等）側の基準がブレている
   - 仕様に対する解釈が割れている
   - 直しても直しきれない設計上の制約がある
4. 止めたら次のいずれかを選ぶ（`dev-fix` F2 のスコープ判断と同じ扱い）:
   - 仕様を再確認してから直す（要件不一致の可能性がある場合）
   - 別 Issue に切り出す（今回のスコープでは対応しない場合）
   - AskUserQuestion で人間に判断を仰ぐ（判断がつかない場合）

> **前提として、D5 ループの収束条件は「Critical 0 件」だけ**（Major は無制限にブロックしない）。何周しても Critical が出続けるなら、実装ではなく指摘基準そのものがブレている可能性を疑う。

> **中〜大規模の verify⇄fix 往復が見込まれる場合は、単体の `/dev-verify` を繰り返すより `/dev-loop` への切替を検討する。** `/dev-loop` は本節と同じ収束チェックを findings のハッシュ比較として自動化しており（`halted-thrashing`）、周回上限（`max_iterations`）も機械的に持っている。
