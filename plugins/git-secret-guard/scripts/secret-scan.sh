...
# diff のファイルヘッダ行 "diff --git a/<path> b/<path>" を追いながら、
# 現在のファイルがテスト/フィクスチャらしいときだけそのファイルの追加行を落とす。
#
# ヘッダの目印を "--- " / "+++ " ではなく "diff --git " にしている理由
# （セキュリティレビューで実測したバイパス経路、2段階で見つかった）:
#
# 1段目: 当初は単に "+++ " で始まる行が来たらヘッダとして扱っていた。しかし
#   「追加された行の中身がたまたま "++ " から始まる」場合、diff 上の見た目は
#   "+"（追加行マーカー）+ "++ 本文" = "+++ 本文" になり、区別が付かない。
#   これを塞ぐため「直前行が "--- " のときだけ次の "+++ " を受理する」形に
#   直した（本物のヘッダは常にこの並びでしか出ないため）。
#
# 2段目: それでも「削除された行の中身が "-- " から始まり、直後の追加行の
#   中身が "++ " から始まる」という**偽の "--- " と偽の "+++ " の組**までは
#   防げていなかった。"-"（削除マーカー）+ "-- 本文" = "--- 本文"、
#   "+"（追加マーカー）+ "++ 本文" = "+++ 本文" が連続すると、1段目の判定を
#   そのまま騙せる。
#
# "--- "/"+++ " はどちらも削除/追加行の正規のプレフィックス文字（"-"/"+"）と
# 衝突しうるので、行の中身側の細工で本物のヘッダに化けられる。
# 一方 "diff --git " は unified diff のハンク内容（+/-/空白のいずれかで
# 必ず始まる）とは異なり、行頭に一切プレフィックスが付かない唯一の行種別。
# 追加/削除された行は必ず "+"/"-" が先頭に付くため、内容がどれだけ
# "diff --git " に似ていても行頭には必ずそのマーカー文字が来る。
# したがって "diff --git " から始まる行は本物のヘッダである保証があり、
# 内容の細工では偽装できない。
...
awk '
  {
    if ($0 ~ /^diff --git /) {
      ...
      # ファイルを本番コードのパスへリネームしつつ本物のシークレットを
      # 追記すると、旧パスの判定のままパターン2・3の検出をすり抜けた）。
      # 一度に "a/<旧パス> b/" までを剥がして新パスだけを残す。
      file = $0
      sub(/^diff --git a\/.* b\//, "", file)
      if (file ~ /^diff --git /) {
        # 上の sub が一致しなかった＝パスの抽出に失敗した。git はファイル名に
        # 非ASCII文字や特殊文字を含むと core.quotepath によりパスを
        # ダブルクォート＋8進エスケープで囲む（例: "a/\346\227\245...".js"）。
        # この形は "a/.* b/" に一致しないため sub が空振りし、file には
        # 未加工の行全体（"diff --git " も旧パスの断片も）がそのまま残る。
        # そのまま lf にして判定すると、旧パス側にたまたま含まれる
        # "tests/" 等の断片で誤って istest=true になりうる（セキュリティ
        # レビューで実測: 日本語ファイル名をテストパスから本番パスへ
        # リネームしつつ追記したシークレットが検出をすり抜けた）。
        # パースに自信が持てないときは安全側（走査する＝除外しない）に倒す。
        istest = 0
        next
      }
      lf = tolower(file)
