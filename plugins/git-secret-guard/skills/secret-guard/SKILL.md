...
- **テスト/フィクスチャファイルの追加行**: diff のファイルパスが `tests?/` `specs?/` `__tests__/`
  ...
  このアンカー変更自体にも実装ミスが1つあり、これも実機再現された:
  リネームでは `diff --git a/<旧パス> b/<新パス>` の形式になる（git はデフォルト
  設定でも類似度50%超で自動検出する）。判定に使う file を旧パス（`a/` 側）のまま
  抽出してしまっていたため、テスト/フィクスチャのパスから本番コードのパスへ
  リネームしつつ末尾に本物のシークレットを追記する操作で、検出を丸ごと
  すり抜けられた。新パス（`b/` 側）を残すよう直した（判定したいのは
  「今そのファイルがどこに置かれるか」なので、リネーム後のパスを使う）。

  それでも1件残っていた: git はファイル名に非ASCII文字を含むと `core.quotepath`
  （既定 `true`）によりパスをダブルクォート＋8進エスケープで囲む（例:
  `diff --git "a/tests/\346..." "b/src/\346..."`）。この形はパス抽出の正規表現に
  一致しないため抽出が空振りし、未加工の行全体（旧パス側の `tests/` という
  断片を含む）がそのまま残ってしまう。それを判定に使うと、新パスが本番コードでも
  旧パスの `tests/` に引っ張られて誤って除外対象になりうる（実機再現: 日本語
  ファイル名をテストパスから本番パスへリネームしつつ追記したシークレットが
  検出をすり抜けた）。パースに自信が持てない（抽出が空振りした）ときは
  安全側＝除外しない（走査する）に倒すよう直した
