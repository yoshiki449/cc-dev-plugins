...
- **テスト/フィクスチャファイルの追加行**: diff のファイルパスが `tests?/` `specs?/` `__tests__/`
  ...
  diff のファイルヘッダ（`--- a/<path>` の直後の `+++ b/<path>`）を追って行う。
  「直前行が `--- ` のときだけ `+++ ` をヘッダとして受理する」という条件が必須で、
  これが無いと「追加行の中身がたまたま `++ ` から始まる」ケース（diff 上は
  `+++ 本文` に見える）を偽ヘッダと誤認し、以降の追加行を丸ごとテスト扱いにして
  検出を漏らすバイパス経路になる（コードレビューで実測）
